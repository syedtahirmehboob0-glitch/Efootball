...
public static class StreetFootballBootstrap
{
    ...
    static Material Mat(Color c){
        ...
        if(shader==null) throw new System.Exception("No compatible Unity material shader is available.");
    ...
    static void MakePitch(){
        ...
        for(int i=-2;i<=2;i++){Cube("Wall"+i,new Vector3(i*5f,1.2f,9),new Vector3(4.8f,2.4f,.35f),new Color(.45f,.35f,.25f));}
        for(int i=0;i<5;i++){float z=-7+i*3.5f;Cube("StreetBuildingL"+i,new Vector3(-15,2,z),new Vector3(2,4,3),new Color(.25f,.22f,.2f));Cube("StreetBuildingR"+i,new Vector3(15,2,z),new Vector3(2,4,3),new Color(.28f,.24f,.2f));}
    }
    static void MakePlayer(){var g=GameObject.CreatePrimitive(PrimitiveType.Capsule);g.name="Pakistan_Player";g.tag="Player";g.transform.position=new Vector3(-5,1,0);g.transform.localScale=new Vector3(.7f,1.1f,.7f);g.GetComponent<Renderer>().sharedMaterial=Mat(new Color(.04f,.25f,.75f));g.AddComponent<Rigidbody>().constraints=RigidbodyConstraints.FreezeRotation;}
    static void MakeBall(){var g=GameObject.CreatePrimitive(PrimitiveType.Sphere);g.name="Ball";g.transform.position=new Vector3(-3,.45f,0);g.transform.localScale=Vector3.one*.65f;g.GetComponent<Renderer>().sharedMaterial=Mat(Color.white);var rb=g.AddComponent<Rigidbody>();rb.mass=.45f;rb.linearDamping=.4f;rb.angularDamping=.2f;}
    ...
    static void MakeAI(string n,Vector3 p,bool blue,Color c){var g=GameObject.CreatePrimitive(PrimitiveType.Capsule);g.name=n;g.transform.position=p;g.transform.localScale=new Vector3(.65f,1,.65f);g.GetComponent<Renderer>().sharedMaterial=Mat(c);g.AddComponent<SimpleFootballAI>().blueTeam=blue;}
    ...
    static void MakeUI(){
        ...
        var game=Object.FindFirstObjectByType<StreetFootballGame>();if(!game)game=new GameObject("GameManager").AddComponent<StreetFootballGame>();
